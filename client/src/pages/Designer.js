...
const Designer = () => {
  ...
  const createWarehouseObject = (e) => {
    ...
    let randomXCoordinate = Math.floor(Math.random() * (1300 - 270 + 1) + 270);
    let randomYCoordinate = Math.floor(Math.random() * (1000 - 200 + 1) + 200);
              ...
              warehouseObjects.map((rect, i) => {
                return (
                  <Rectangle
                    ...
                    onChange={(newAttrs) => {
                      const rects = warehouseObjects.slice();
                      rects[i] = newAttrs;
